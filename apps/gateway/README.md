...
Part of the [whappr monorepo](../../README.md).
...
Run these with `-w apps/gateway` from the repo root, or without the flag from inside this
directory.
...
Repo-wide `typecheck` and `check` (Biome lint + format + import sort) scripts live at the
[repo root](../../README.md#root-scripts) and cover every package, this one included.
